...
def test_build_from_base_sintetico():
    ...
    assert list(t.columns) == tienda.COLS
    assert set(t["tienda"]) == {"JP", "SM"} and len(t) == 3          # C no tiene stock ni OO en ninguna
    jp_a = t[(t.sku == "A") & (t.tienda == "JP")].iloc[0]
    assert jp_a.vta_soles_sem == 59.9                                  # soles por tienda (2026-09-12)
    ...
    assert sm_b.vta_soles_sem == 0.0                                   # base sin '{t} Vta S/.' → 0
